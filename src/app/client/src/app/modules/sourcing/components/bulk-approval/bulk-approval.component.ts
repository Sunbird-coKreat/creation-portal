...
export class BulkApprovalComponent implements OnInit, OnChanges {
  ...
  public adapterContents: Array<any>;
      ...
      this.bulkJobService.searchContentWithProcessId(this.bulkApprove.process_id, this.bulkApprove.type).subscribe((res: any) => {
        ...
        if (res.result) {
          ...
          if (contents.length) {
            ...
            this.adapterContents = contents;
            overallStats['approve_success'] = _.filter(this.adapterContents, content => content.status === 'Live').length;
            // tslint:disable-next-line:max-line-length
            overallStats['approve_failed'] = _.filter(this.adapterContents, content => content.status === 'Failed').length;
  ...
  prepareTableData() {
    const failedContent = _.filter(this.adapterContents, content => content.status === 'Failed');
    if (!this.programContext.target_type || this.programContext.target_type === 'collections') {
  ...
  downloadFailedContentReport() {
    if (!this.adapterContents) {
      this.bulkJobService.searchContentWithProcessId(this.bulkApprove.process_id, this.bulkApprove.type).subscribe((res: any) => {
        if (res.result && res.result.content && res.result.content.length) {
          this.adapterContents = _.get(res, 'result.content');
