...
export class ContentEditorComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    ...
    this.telemetryInteractCdata = this.programTelemetryService.getTelemetryInteractCdata(this.contentEditorComponentInput.programContext.programId, 'Program');
    // tslint:disable-next-line:max-line-length
  ...
  ngAfterViewInit() {
    const telemetryCdata = [{ 'type': 'Program', 'id': this.contentEditorComponentInput.programContext.programId }];
    setTimeout(() => {
  ...
  publichContent() {
    this.helperService.publishContent(this.contentEditorComponentInput.contentId, this.userService.userProfile.userId)
       .subscribe(res => {
        if (this.sessionContext.collection && this.contentEditorComponentInput.unitIdentifier) {
          // tslint:disable-next-line:max-line-length
          this.collectionHierarchyService.addResourceToHierarchy(this.sessionContext.collection, this.contentEditorComponentInput.unitIdentifier, res.result.content_id)
          .subscribe((data) => {
