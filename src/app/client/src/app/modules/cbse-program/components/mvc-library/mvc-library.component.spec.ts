...
describe('MvcLibraryComponent', () => {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      ...
      providers: [{ provide: Router }, { provide: ActivatedRoute, useValue: fakeActivatedRoute }, ConfigService,
        ...
        DatePipe, ActionService, ProgramsService, CbseProgramService],
  ...
  it('#initialize() should set collection and program details data', () => {
    ...
    spyOn(component, 'plusMinusGradeLevel').and.returnValue(mockMvcLibraryData.plusMinusGradeLevelRes);
    ...
    expect(component.plusMinusGradeLevel).toHaveBeenCalled();
    expect(component.filterData.subjects).toEqual(mockMvcLibraryData.collectionHierarchyReadSuccess.result.content.subject);
    expect(component.filterData.gradeLevels).toEqual(mockMvcLibraryData.plusMinusGradeLevelRes);
    expect(component.filterData.contentTypes).toEqual(mockMvcLibraryData.programReadSuccess.result.content_types);
    expect(component.activeFilterData).toEqual(component.filterData);
    expect(component.collectionHierarchy).not.toBeUndefined();
  });

  it('#initialize() should throw error message when collection hierarchy API failed', () => {
    const actionService: ActionService = TestBed.get(ActionService);
  ...
  it('#showResourceTemplate() should open collection hierarchy popup when event #beforeMove', () => {
    ...
    expect(component.showLargeModal).toBe(true);
