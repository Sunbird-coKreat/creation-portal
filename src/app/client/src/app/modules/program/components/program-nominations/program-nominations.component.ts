...
export class ProgramNominationsComponent implements OnInit, AfterViewInit, OnDestroy {
 ...
 public logTelemetryImpressionEvent(data?, keyName?, type?) {
  ...
  getProgramCollection (preferencefilters?, target_type?) {
    return this.collectionHierarchyService.getCollectionWithProgramId(this.programId, this.programDetails.target_collection_category, preferencefilters, false, target_type).pipe(
      tap((response: any) => {
