...
const KONG_LOGGEDIN_DEVICE_REGISTER_TOKEN =
  require("./environmentVariablesHelper.js").SUNBIRD_PORTAL_API_AUTH_TOKEN;
const KONG_REFRESH_TOKEN_API =
  require("./environmentVariablesHelper.js").sunbird_kong_refresh_token_api;
const USE_SUNBIRD_KONG_TOKEN =
  require("./environmentVariablesHelper.js").USE_SUNBIRD_KONG_TOKEN;
...
const getKongAccessToken = (req, cb) => {
  if (USE_SUNBIRD_KONG_TOKEN === "true") {
    try {
      _log(
        req,
        "KONG_TOKEN refresh_token:: requesting kong auth token for session id [ " +
      ...
      sendRequest(options)
        .then((response) => {
          ...
          if (_.get(responseData, "params.status") === "successful") {
            _log(
              req,
              "KONG_TOKEN refresh_token :: Successfully generated kong refresh auth token for session id [ " +
                _.get(req, "sessionID") || "no_key" + " ]"
            );
            req.session.cookie.maxAge =
              _.get(responseData.result, "expires_in") * 1000;
            req.session.cookie.expires = new Date(
              Date.now() + _.get(responseData.result, "expires_in") * 1000
            );
            req.session[KONG_ACCESS_TOKEN] = _.get(
              responseData,
              "result.access_token"
            );
            ...
            _log(
              req,
              "KONG_TOKEN refresh_token :: Failed to generate kong auth token for session id [ " +
                _.get(req, "sessionID") || "no_key" + " ]"
            );
            cb(true);
          }
      ...
      _log(
        req,
        "KONG_TOKEN refresh_token :: Method failed to generate kong auth token for session id [ " +
          _.get(req, "sessionID") || "no_key" + " ]. Error => " + error
      );
      cb(error);
    }
